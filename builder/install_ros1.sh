...
# install_ros1.sh
#
# Installs ROS1 and related packages into the image.
# Must run as root.
#
# Usage: install_ros1.sh <ros_distro>
#   ros_distro: e.g. noetic
...
#
# Removes legacy ROS1 deb lines (and their GPG keys) from apt sources.
# Needed because on 2025-06-01 the key/repo management moved to ros-apt-source.
# Ref: https://discourse.ros.org/t/ros-signing-key-migration-guide/43937
sanitize() {
    local version_codename="${1}"
    local url="http://packages.ros.org/ros/ubuntu"
...
# --------------------------------------------------------------------------------------------------
# ROS1 packages to install.
# For a list of all packages in each variant (ros_core, ros_base, robot, perception, desktop,
# desktop_full, simulators, viz), see https://github.com/ros/metapackages.
...
# rosmon is a useful process manager for ROS1 nodes — uncomment if needed.
# --------------------------------------------------------------------------------------------------
packages=(
  ...
  # ros-${ROS_DISTRO}-rosmon  # process manager for ROS1 nodes; uncomment if needed
)

# --------------------------------------------------------------------------------------------------
# Remove legacy apt sources and install ROS1.
# --------------------------------------------------------------------------------------------------
sanitize "${version_codename}"
...
ros_list_file=""
ros_apt_source_package="ros-apt-source"
...
if ! dpkg --status "${ros_apt_source_package}" >/dev/null 2>&1; then
    ...
    curl --fail --silent --show-error --location \
        ...
        handle_error 1 "Downloading or dearmoring the ROS1 GPG key failed"
    ...
    log info "Adding ROS1 deb line to '${ros_list_file}'"
    echo "${ros_deb_line}" | tee "${ros_list_file}" >/dev/null || \
        handle_error 1 "Failed to write ROS1 deb line to '${ros_list_file}'"
